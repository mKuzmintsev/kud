#!/bin/bash
#Проверка соединений для одноэтажных вагонов ППФ, кроме ресторана: 4447.07, 4458.12, 4462, 4463
...
if grep '\.7\.[0-9]\{1,\} = STRING: "Slot0/7"' $rnd > /dev/null && grep "\.7\.[0-9]\{1,\} = Hex-STRING: $mac3" $rnd > /dev/null
then
...
if grep '\.10\.[0-9]\{1,\} = STRING: "Slot0/8"' $rnd > /dev/null && grep "\.10\.[0-9]\{1,\} = Hex-STRING: $mac3" $rnd > /dev/null
then
